...
local elec_read_prcess_prcess=function(mac,code,rssi,data_type,data)
	print("elec ack msg !!!!")
...
local elec_read_child=function(mac)
		...
		pack:set(mac,0x0b,0x0702,zcl.PROFILE_CMD,zcl.READ,0,0,zcl.cmdtostr(zcl.READ,read_req))
		
		zbdev.send(mac,0x0900,pack,pack:size(),elec_read_prcess_prcess,nil)
end


local  elec_notify=function(mac,child_id,code)
...
local elec_zigbee_recive =function (mac,child_id,rssi,data_type,data)
...
local elec_send_prcess_prcess=function(mac,code,rssi,data_type,data)
	print("elec_send_prcess_prcess code =" .. code )
end

local elec_coap_recive=function(mac,child_id,fd,token,pdu)
...
zigbee_obj_register_devmodle("dev_elec",dev_elec,elec_coap_recive,elec_zigbee_recive,elec_notify,0)
